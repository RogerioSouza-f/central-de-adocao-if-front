                    ...
                    {animal.photos && animal.photos.length > 0 && animal.photos[0] && (
                        <div className="detail-section">
                            ...
                            <div className="photo-gallery">
                                {animal.photos.filter(p => p.trim() !== '').map((photo, index) => (
                                    <div key={index} className="gallery-item">
                                        <img
                                            ...
                                            onError={(e) => {
                                                ...
                                                e.target.parentElement.innerHTML = `<div style="display: flex; align-items: center; justify-content: center; height: 100%; background: rgba(239, 68, 68, 0.2); color: #ef4444; font-size: 3rem;">${animal.species === 'Cachorro' ? '🐕' : '🐱'}</div>`;
                                            }}
                                        />
                                    </div>
                                ))}
